...
log_name = './log/log_seasonality_' + date + '.txt'
...
for i in range(len(season)):
    path = './data/Seasonality/K_CWQI_preprocessed_230613_' + season[i] + '.csv'
    ...
    plt.title('Correlations (' + season[i] + ')', fontsize=35)
    sns.heatmap(corr, vmin=-1.0, vmax=1.0, square=True, annot=True, cmap='Blues', linewidths=.5, annot_kws={'size':25}, fmt='.2f')
    fig1_name = './result/Seasonality/fig1_230613_' + season[i] + '.png'
    print('Saving ', fig1_name)
    ...
    print(season[i] + "_Best Average Accuracy: {0:.4f}".format(rfc_grid.best_score_))
    print(season[i] + "_Best Hyperparameter: ", rfc_grid.best_params_)
    ...
    grid_search_result = './result/Seasonality/grid_search_result_230613_' + season[i] + '.csv'
    ...
    plt.title('Feature importance of Random Forest Regression (' + season[i] + ')', fontsize=35)
    plt.xlabel('Features', fontsize=20)
    plt.xticks(fontsize=18)
    plt.ylabel('Importances', fontsize=20)
    plt.yticks(fontsize=18)
    plt.ylim((0.0, 1.0))
    importances_fig = './result/Seasonality/rf_importances_230613_' + season[i] + '.png'
    print('Saving ', importances_fig)
    ...
    prediction_csv = './result/Seasonality/rf_regression_predict_230613_' + season[i] + '.csv'
    ...
    plt.xlim((30, 100))
    plt.ylim((30, 100))
    plt.title('Result (' + season[i] + ')', fontsize=30)
    rf_result = './result/Seasonality/rf_results_230613_' + season[i] + '.png'
