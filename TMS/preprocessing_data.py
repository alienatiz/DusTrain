...
data_oct = pd.read_excel(orig_data_path + 'tms_oct.xlsx')
data_nov_1w = pd.read_excel(orig_data_path + 'tms_nov_1w.xlsx')
...
data_oct.replace(['보수중', '기기점검', '측정자료확인중(가동중지)', '측정자료확인중'], np.NAN, inplace=True)
data_nov_1w.replace(['보수중', '기기점검', '측정자료확인중(가동중지)', '측정자료확인중'], np.NAN, inplace=True)
...
data_oct_repl = data_oct[~data_oct['stack_code'].isin(delete_stack_list)]
data_nov_1w_repl = data_nov_1w[~data_nov_1w['stack_code'].isin(delete_stack_list)]
...
# data_oct_repl.to_csv(orig_data_path + 'data_oct.csv', sep=',', na_rep='NaN', index=False, encoding='UTF-8-sig')
data_nov_1w_repl.to_csv(orig_data_path + 'data_nov_1week.csv', sep=',', na_rep='NaN', index=False, encoding='UTF-8-sig')
